# Sub-agent harness — lessons 12–13 + write_file gated by human_confirm.py (lesson 14).
import json
import re
import urllib.error
...
from human_confirm import confirm_action
...
MAX_GREP_MATCHES = 25
MAX_EMPTY_RETRIES = 2
DEFAULT_OUTPUT_PATH = "output/summary.md"  # where maybe_save_answer writes
...
def write_file(path_arg: str, content: str) -> str:
    """Destructive tool — always requires human approval before touching disk."""
    target = safe_path(path_arg)
    if target is None:
        return f"Error: invalid path {path_arg!r} — must stay inside workspace/"
...
def run_manager(user_question: str) -> str:
    ...
    answer = run_writer(user_question, facts)

    print("[manager] ← writer done")
    # Lesson 14 — Python blocks on input() before write_file touches disk
